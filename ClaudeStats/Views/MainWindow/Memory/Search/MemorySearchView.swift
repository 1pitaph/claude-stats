...
struct MemorySearchView: View {
        ...
        if let traceID = store.codeLastTraceID {
            HStack(spacing: 10) {
                ...
                Button {
                    ...
                    Label("Trace", systemImage: "arrow.right")
                }
                .controlSize(.small)
                MemoryCopyButton(value: traceID, label: "Copy Trace", systemImage: "link")
            }
            .padding(10)
    ...
    private var canonicalSection: some View {
        MemorySection(title: "Canonical Memories", count: store.codeSearchResults.count, symbol: "text.badge.checkmark") {
    ...
    private var graphSection: some View {
        MemorySection(title: "Graph Facts", count: store.codeGraphResults.count, symbol: "point.3.connected.trianglepath.dotted") {
    ...
    private var sourceSection: some View {
        MemorySection(title: "Sources", count: store.codeSourceResults.count, symbol: "doc.text.magnifyingglass") {
    ...
    private var contextPreview: some View {
        if let pack = store.codeContextPack {
            MemorySection(title: "Context Preview", count: contextCount(pack), symbol: "doc.text") {
    ...
    private var tracePreview: some View {
        if let trace = store.codeTrace {
            MemorySection(title: "Trace", count: trace.memoryUsage.count, symbol: "list.bullet.clipboard") {
